...
export default function Admin() {
  ...
  const todayAppts = getAppointmentsByDate(format(new Date(), 'yyyy-MM-dd'));
  ...
  const handleSave = async () => {
    ...
    if (modal.mode === 'create') {
      await createAppointment({ ...formData, serviceName: service.name, price: service.price, userId: 'admin-created' });
    } else {
      await updateAppointment(modal.appt.id, { ...formData, serviceName: service.name, price: service.price });
    }
    setModal(null);
  };

  const handleDelete = async (id) => {
    if (window.confirm('Eliminar este turno?')) await deleteAppointment(id);
  };
    ...
    <div className="admin-page page-wrapper">
      <div className="admin-header">
        ...
        <div className="admin-stats">
          ...
          <div className="stat">
            <span className="stat-n">{todayAppts.length}</span>
      ...
      <div className="admin-layout">
        <div className="admin-cal-col">
          ...
          <Calendar selectedDate={selectedDate} onSelectDate={setSelectedDate} appointments={appointments} />
        ...
        <div className="admin-table-col">
          <div className="table-header">
            ...
            <div className="table-filter">
              ...
              {filterDate && <button className="btn-ghost" onClick={() => setFilterDate('')} style={{fontSize:'12px'}}>X Limpiar</button>}
            </div>
          </div>
          <div className="appts-table">
            <div className="table-head">
              <span>Cliente</span><span>Servicio</span><span>Fecha</span><span>Hora</span><span>Estado</span><span>Acciones</span>
            </div>
            {filteredAll.length === 0 ? (
              <div className="table-empty">No hay turnos para mostrar.</div>
            ) : (
              filteredAll.sort((a, b) => (a.date + a.time) > (b.date + b.time) ? 1 : -1).map(appt => (
        ...
        <div className="modal-overlay" onClick={() => setModal(null)}>
          <div className="modal-box card" onClick={e => e.stopPropagation()}>
            <div className="modal-header">
              ...
              <button className="btn-ghost" onClick={() => setModal(null)}>X</button>
            </div>
            <div className="modal-form">
              <div className="form-group">
                <label>Nombre del cliente</label>
                <input name="userName" value={formData.userName || ''} onChange={handleFormChange} placeholder="Juan Garcia" />
              </div>
              <div className="form-group">
